...
def display():
    ...
    l_pos = np.array(
        (10*m.sin(angle), 0, 10*m.cos(angle)), dtype=np.float32
    )
    L = l_pos * (1/m.sqrt(l_pos.dot(l_pos)))
    L = L.reshape((-1, 1))
    # for i in range(len(colors)):
    #     s = max(0, normals[i, 0:3].dot(L))
    #     colors[i] = s * red

    colors[:, 0:3] = np.maximum(0, normals[:, 0:3].dot(L)) * red
