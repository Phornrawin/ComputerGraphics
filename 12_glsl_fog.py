...
void main(){
    ...
    frag_code = b'''
#version 330
uniform vec3 l_pos, l_dcol, l_scol, e_pos;
uniform float m_kd, m_ks, m_shininess;
...
void main(){
    ...
    specular = pow(HdotN, m_shininess);
    if (diffuse == 0)
        specular = 0;
    edge = max(dot(N, V), 0);
    ...
    if (edge < 0.3)
        ...
        edge = 1;
    vec3 color = egde * (diff_color + spec_color) * fCol;
